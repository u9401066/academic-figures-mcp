...
- `pyproject.toml`
- `src/__init__.py` if it defines `__version__`
- `Dockerfile` image version label if `Dockerfile` exists
...
VSIX install/activation smoke is required for release. On Linux with a display stack, prefer:
<execute_command>
<command>(cd vscode-extension && xvfb-run -a npm run test:install-smoke -- --require-activation)</command>
</execute_command>

<execute_command>
<command>uv run python scripts/package_smoke.py</command>
</execute_command>
...
<execute_command>
<command>python scripts/get_version.py --strict-semver</command>
</execute_command>

Create a release commit:
<execute_command>
<command>VERSION="$(python scripts/get_version.py --strict-semver)"; git commit -m "Release $VERSION"</command>
</execute_command>
...
<execute_command>
<command>VERSION="$(python scripts/get_version.py --strict-semver)"; git tag -a "v$VERSION" -m "Release v$VERSION"</command>
</execute_command>

<execute_command>
<command>VERSION="$(python scripts/get_version.py --strict-semver)"; git push origin "v$VERSION"</command>
</execute_command>
