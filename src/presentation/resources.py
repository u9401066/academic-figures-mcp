    ...
    return json.dumps(
        {
            ...
            "tracked_ecosystem": [
                "D2", "Mermaid", "Kroki", "Matplotlib", "SciencePlots",
                "FigureFirst", "CairoSVG", "Excalidraw", "tldraw",
            ],
        },
        indent=2,
        ensure_ascii=False,
...
def renderer_ecosystem_resource() -> str:
    ...
    return json.dumps(
        {
            "shortlist": [
                {"name": "D2", "role": "primary structured diagram DSL",
                 "integration": ["mcp", "vscode-extension", "backend-rendering"]},
                {"name": "Mermaid", "role": "lightweight flowchart and timeline DSL",
                 "integration": ["mcp", "vscode-extension", "backend-rendering"]},
                {"name": "Kroki", "role": "optional compatibility render gateway",
                 "integration": ["mcp", "backend-rendering"]},
                {"name": "Matplotlib", "role": "canonical chart engine",
                 "integration": ["mcp", "backend-rendering"]},
                {"name": "SciencePlots", "role": "publication-style preset source",
                 "integration": ["mcp", "backend-rendering"]},
                {"name": "FigureFirst", "role": "multi-panel SVG layout pattern",
                 "integration": ["backend-rendering"]},
                {"name": "CairoSVG", "role": "SVG export and derivative conversion",
                 "integration": ["backend-rendering"]},
                {"name": "Excalidraw", "role": "lightweight editable scene format",
                 "integration": ["vscode-extension"]},
                {"name": "tldraw", "role": "advanced interactive editor SDK",
                 "integration": ["vscode-extension"]},
            ],
            "capabilities": {
